...
description: 'Generate a project-local verification skill that drives your app the way a user does, in any language, framework, or platform. Use for /create-verification-skill, "make a verification skill for this repo", or when a project has no scripted way to prove UI/CLI/service behavior.'
---
...
Write `.agents/skills/verify-<app>/SKILL.md` with YAML frontmatter (`name: verify-<app>` and a `description` that names the app, the surface, and when to reach for it, and without frontmatter the skill never registers) and these sections, each grounded in what the interview actually found (no placeholders left):
