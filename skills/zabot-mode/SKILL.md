...
**Start every multi-step task with a todolist whose first todo item is to read the Principles section below in full.** Add the matched playbook's steps verbatim immediately after that item, before task-specific items. The principles ground every trigger here. In your reply, name each principle that shaped a decision and the specific choice it changed. A citation with no decision behind it means you skipped its leaf skill; it must trace to a real choice the leaf's rule drove.
...
Use the built-in explorer role for repository discovery, default for general work, and worker for bounded implementation. Give every delegation a unique `task_name`. Set `model` only when a specific available model is needed, and pair it with an appropriate `reasoning_effort`; otherwise inherit the parent's settings.
...
Zabot Mode is the workflow controller. It selects and runs the matching playbook, and owns the work it delegates. A project verification skill is separate: it is the generated `.agents/skills/verify-<app>/SKILL.md` that drives the real app and captures runtime evidence. When a playbook needs to launch, drive, or measure the app, use that project verification skill.

After the required principle-reading item, the next todolist actions are the matched playbook's steps, copied verbatim before task-specific todos and before you reason about the task. The failure mode is reading a playbook then writing a bespoke plan that drops its named steps (`architect`, the throughput checkpoint). A step you choose not to do stays in the list with a one-line `skip: <reason>`; skipping silently is not allowed. Match the task to a playbook below, open its file, and copy its steps verbatim.
